...
import { ShowOnMap7MinService } from './show-on-map-7min.service';
...
@Component({
	selector: 'showonmap7min',
	templateUrl: './show-on-map-7min.component.html',
	styleUrls: ['show-on-map-7min.component.scss'],
})
export class ShowOnMap7MinComponent implements OnInit {
	...
	*/
	...
	constructor(private renderer: Renderer2, private elementRef: ElementRef, private _service: ShowOnMap7MinService) {
	}
	...
	initMap() {
		...
		let spatialResult = []
		 this._service.getSpatialData().subscribe(res => {
			if(res) {
			spatialResult = res;
	...
	addMarkerWithHeading(terminal: any) {
		if (terminal) {
			...
			const imageObject = new style.Icon({
				...
				src: '../../../../assets/media/map-locazilation/map-localization1.png',
				scale: 0.7,
				snapToPixel: false,
				zIndex: 11
			});
