...
import { ShowOnMapStdbscanService } from './show-on-map-stdbscan.service';
import { STDbscanService } from './stdbscan-algo.service';
...
@Component({
	selector: 'showonmapstdbscan',
	templateUrl: './show-on-map-stdbscan.component.html',
	styleUrls: ['show-on-map-stdbscan.component.scss'],
})
export class ShowOnMapStdbsanComponent implements OnInit {
	minInterval = 60;
	...
	*/
	...
	constructor(private renderer: Renderer2, private elementRef: ElementRef, private _service: ShowOnMapStdbscanService, private activatedRoute: ActivatedRoute,
	...
	initMap() {
		...
		this.currentSpatialData.forEach(d => {
	...
	addMarkerWithHeading(terminal: any) {
		if (terminal) {
			...
			const imageObject = new style.Icon({
				...
				src: '../../../../assets/media/map-locazilation/map-localization' + (detail.cluster != 0 ? (detail.cluster % 11) + 1 : 0) + '.png',
				scale: 0.7,
				snapToPixel: false,
				zIndex: 11
			});
			...
			const tName = terminal.cluster.toString();
