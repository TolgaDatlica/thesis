...
import { PositionsOnMapService } from './positionsonmap.service';
...
@Component({
	selector: 'positionsonmap',
	templateUrl: './positionsonmap.component.html',
	styleUrls: ['positionsonmap.component.scss'],
})
export class PositionsOnMapComponent implements OnInit {
	@Input() minInterval: number;
	mapwrapper: string = 'mapwrapper';
	mapid: string = 'map';
	...
	*/
	...
	constructor(private renderer: Renderer2, private elementRef: ElementRef, private _service: PositionsOnMapService, private activatedRoute: ActivatedRoute) {
	}

	ngOnInit(): void {
		this.mapid = 'map' + this.minInterval;
		this.mapwrapper = 'mapwrapper' + this.minInterval;
		setTimeout(() => {
	...
	updatemap() {
		setTimeout(() => {
			...
			if (document.getElementById(this.mapid)) {
				document.getElementById(this.mapid).remove();
				...
				this.renderer.setAttribute(divMap, 'id', this.mapid);
				document.getElementById(this.mapwrapper).appendChild(divMap);
				document.getElementById(this.mapid).style.height =
					(document.getElementsByTagName('html')[0].clientHeight - 400) + 'px';
			}
			/* İnit Map */
			this.initMap();
